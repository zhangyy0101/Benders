...
def execute_run(*,instance_id,instance_path,expected_digest,method,configuration,seed,budget,threads,mip_gap,alloc_domain,handling_rate_scale,outbound_policy,output,save_solutions=False,method_runner=run_method,command=None):
    ...
    strengthening=solver.get("master_strengthening") or solver.get("phase3_bbc",{}).get("master_diagnostics",{}).get("master_strengthening") if status["status"]!="EXCEPTION" else None;evaluation_safe=_json_safe(evaluation) if evaluation else None;result={"schema_version":SCHEMA_VERSION,"run_id":run_id,"identity":{"instance_id":instance_id,**identity,"configuration_name":configuration["configuration_name"],"configuration_version":configuration["configuration_version"],"configuration_status":configuration["status"],"resolved_algorithm_label":resolved_algorithm_label(configuration)},"configuration":configuration,"status":status,"timing":timing,"optimization":optimization,"master_strengthening":_json_safe(strengthening),"method_diagnostics":_json_safe(_method_diagnostics(solver)) if status["status"]!="EXCEPTION" else None,"evaluation":evaluation_safe,"environment":environment_metadata(command)}
