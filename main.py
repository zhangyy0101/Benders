...
def main():
    ...
    parser.add_argument("--instance",choices=INSTANCES,default="3new6old"); parser.add_argument("--phase1-time",type=float,default=20); parser.add_argument("--lns-time",type=float,default=45); parser.add_argument("--phase3-time",type=float,default=20); parser.add_argument("--attribute-time",type=float,default=20); parser.add_argument("--attribute-epsilon",type=float,default=.01); parser.add_argument("--alloc-domain",choices=("integer","continuous"),default="integer"); parser.add_argument("--handling-rate-scale",type=float,default=1.0); parser.add_argument("--no-valid-inequalities",action="store_true"); parser.add_argument("--no-alns",action="store_true"); parser.add_argument("--no-proof",action="store_true"); parser.add_argument("--no-attribute-refinement",action="store_true"); parser.add_argument("--attribute-components",choices=("all","pod","weight","height"),default="all"); parser.add_argument("--seed",type=int,default=0); parser.add_argument("--output-root",default="outputs"); parser.add_argument("--verbose",action="store_true"); args=parser.parse_args()
    aw=AttributeRefinementWeights(pod_spread=4.0 if args.attribute_components in ("all","pod") else 0.0,weight_spread=3.0 if args.attribute_components in ("all","weight") else 0.0,height_mix=10.0 if args.attribute_components in ("all","height") else 0.0); weights=Weights(attribute=aw)
    data=prepare_instance(INSTANCES[args.instance](),args.handling_rate_scale); result=solve_strengthened_mip_alns(data,weights,phase1_time=args.phase1_time,lns_time=args.lns_time,phase3_time=args.phase3_time,attribute_time=args.attribute_time,attribute_epsilon=args.attribute_epsilon,alloc_domain=args.alloc_domain,add_valid_inequalities=not args.no_valid_inequalities,seed=args.seed,verbose=args.verbose,enable_alns=not args.no_alns,enable_proof=not args.no_proof,enable_refinement=not args.no_attribute_refinement)
    ...
    if result.get("ok"):
        ...
        with open(os.path.join(out,"alns_iterations.json"),"w",encoding="utf8") as f: json.dump(_jsonable(result["phase2_alns"].get("iterations",[])),f,indent=2)
        summary={"algorithm":"strengthened_mip_alns","instance":args.instance,"settings":{"alloc_domain":args.alloc_domain,"handling_rate_base":data["handling_rate_base"],"handling_rate_scale":args.handling_rate_scale,"handling_rate_source":data["handling_rate_source"],"valid_inequalities":not args.no_valid_inequalities,"alns":not args.no_alns,"proof":not args.no_proof,"attribute_refinement":not args.no_attribute_refinement,"attribute_components":args.attribute_components},"phase1_core_mip":_phase_summary(result["phase1_core_mip"]),"phase2_alns":{**_phase_summary(result["phase2_alns"]),"iteration_count":len(result["phase2_alns"].get("iterations",[])),"operators":result["phase2_alns"].get("operators",{}),"iterations_file":"alns_iterations.json"},"phase3_proof_mip":_phase_summary(result["phase3_proof_mip"]),"core_best":{"ub":core["ub"],"lb":core["lb"],"gap":core["gap"],"solution_file":"core_best_solution.json"},"attribute_refinement":{**{k:v for k,v in ref.items() if k!="solution"},"solution_file":"attribute_refined_solution.json"}}
