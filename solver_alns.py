...
def _period_pair_mass(data,solution,pair,periods):
    i,j=pair;G=ship_groups(data,j);return sum(float(solution.get("x",{}).get((i,j,n),0))+sum(float(solution.get("alloc_boxes",{}).get((i,j,g,n),0))+float(solution.get("din",{}).get((j,g,i,n),0)) for g in G) for n in periods)
    ...
    if operator=="active":pool=sorted(active)
        ...
        def contribution(q):
            ...
            if solution.get("din") is not None:
                flows={n:sum(float(solution["din"].get((j,g,i,n),0)) for g in ship_groups(data,j)) for n in data["N"]}
            else:flows={n:sum(float(solution.get("alloc_boxes",{}).get((i,j,g,n),0)) for g in ship_groups(data,j)) for n in data["N"]}
...
def select_destination_pairs(data,solution,source_pairs,operator,count,rng,context):
    ...
    for i,j in all_pair_keys(data):
        ...
        compatible=[g for g in ship_groups(data,j) if group_size(data,g)==int(data["Fixed_Bay_Mode"][i]) and required_reserve(data,j,g,final,"continuous")>1e-9]
...
def build_released_variable_keys(data,released_pairs,free_periods):
    pairs=set(released_pairs);periods=set(free_periods);rx={(i,j,n) for i,j in pairs for n in data["N"] if n in periods};ra={(i,j,g,n) for i,j in pairs for g in ship_groups(data,j) for n in data["N"] if n in periods};return rx,ra
...
def adaptive_lns(data,weights,start_solution,*,time_limit=10,repair_time=2,repair_gap=.03,min_destroy=.1,max_destroy=.35,restarts=1,stall_iters=10,destination_ratio=1.0,min_repair_time=None,max_repair_time=None,alloc_domain="integer",add_valid_inequalities=True,concentration_enabled=True,seed=0):
    ...
    while time.perf_counter()<deadline:
        ...
        if improved_best:anytime.append({"time":time.perf_counter()-beg,"phase":"alns","source":"best_improvement","ub":best_cost,"lb":None})
        elif accepted and improved_current:anytime.append({"time":time.perf_counter()-beg,"phase":"alns","source":"accepted_improvement","ub":current_cost,"lb":None})
    ...
    terminated_by_deadline=terminated_by_deadline or time.perf_counter()>=deadline;elapsed=time.perf_counter()-beg;return {"anytime_trace":canonicalize(anytime,elapsed,best_cost,None),"start_ub":start_eval["core_cost"],"best_ub":best_cost,"improvement":start_eval["core_cost"]-best_cost,"best_solution":best,"iterations":log,"operator_stats":stats,"restarts":restart_count,"persistent_repair_model":True,"terminated_by_deadline":terminated_by_deadline,"runtime":elapsed}
