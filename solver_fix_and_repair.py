...
def solve_aggregate_guided_fix_and_repair(
    ...
    *,
    ...
    if incumbent_solution is None:
        ...
        alloc_support = {}
        for (i, j, g, _n), value in incumbent_solution.get("alloc_boxes", {}).items():
            if value > 1e-7:
                item = alloc_support.setdefault((j, g, i), {"max": 0.0, "sum": 0.0})
                item["max"] = max(item["max"], float(value)); item["sum"] += float(value)
        x_support = {}
        for (i, j, _n), value in incumbent_solution.get("x", {}).items():
            if value > 1e-7: x_support[j, i] = max(x_support.get((j, i), 0.0), float(value))
        guide = {"ok": True, "source": "bbc_incumbent_support", "runtime": 0.0,
                 "x": incumbent_solution.get("x", {}), "alloc_boxes": incumbent_solution.get("alloc_boxes", {}),
                 "diagnostics": {"aggregate_flow_by_ship_size_block": {},
    ...
    full_pair_count = sum(sum(int(data["Fixed_Bay_Mode"][i]) == group_size(data, g) for i in data["I_list"])
    ...
    previous_solution = incumbent_solution
    first_level = 0 if full_pair_count <= 1000 else 1 if incumbent_solution is not None else 2
    for attempt_index in range(max(0, int(max_expansions)) + 1):
        ...
        repair = solve_restricted_monolithic_repair(
            ...
            add_valid_inequalities=add_valid_inequalities, concentration_enabled=concentration_enabled, solution_limit=10,
            validation_time_reserve=min(8.0, .30 * max(0.0, float(time_limit))) if canonical_oracle else 0.0,
    ...
    return {"ok": True, "status_name": best["status_name"], "runtime": runtime,
            ...
            "selected_attempt": selected, "oracle_consistent": True if best.get("oracle_status_name")=="OPTIMAL" else None,
            "anytime_trace": trace}
