...
def evaluate_solution(data,weights,solution,*,alloc_domain="integer",concentration_enabled=True):
    scales=objective_scales(data);pressure=outbound_pressure(data);G=groups(data);first=first_stage_cost(data,weights,solution["x"],solution["alloc_boxes"],alloc_domain=alloc_domain,concentration_enabled=concentration_enabled);distance=sum(float(data["Dist"][j,k])*solution["in_share"][j,k,g,n] for j in data["J_new"] for k in data["K"] for g in G for n in data["N"]);balance=sum(solution["g_bal"].values());conflict=sum(float(pressure[k,n])*solution["in_share"][j,k,g,n] for j in data["J_new"] for k in data["K"] for g in G for n in data["N"]);factor=scale_factor(weights);distance_cost=factor*weights.sub.dist*distance/scales["distance"];balance_cost=factor*weights.sub.balance*balance/scales["balance"];conflict_cost=factor*weights.sub.conflict*conflict/scales["conflict"];rec=distance_cost+balance_cost+conflict_cost;con=first["concentration"];return {"open_cost":first["open_cost"],"concentration_raw_used_bays":con["raw_used_bays"],"concentration_normalized":con["normalized"],"concentration_cost":first["concentration_cost"],"concentration":con,"recourse_cost":rec,"core_cost":first["total"]+rec,"distance_raw":distance,"balance_raw":balance,"conflict_raw":conflict,"distance_cost":distance_cost,"balance_cost":balance_cost,"conflict_cost":conflict_cost}
