...
def validate_solution(data,solution,*,alloc_domain="integer",add_valid_inequalities=True,concentration_enabled=True,tolerance=1e-5):
    I,J,G,N,K=data["I_list"],data["J_new"],groups(data),data["N"],data["K"];pairs=ship_group_pairs(data);alpha=float(data["Alpha"]);rem=remaining_capacity(data);viol={}
    ...
    for j in J:
     ...
     for i in I:
      for n in N:rec("handling",alpha*sum(val("din",(j,g,i,n)) for g in ship_groups(data,j))-float(data["Bay_Handling_Rate"][i,n])*float(data["Intervals"][n]["dur"])*val("x",(i,j,n)));rec("alloc_x",sum(val("alloc_boxes",(i,j,g,n)) for g in ship_groups(data,j))-rem[i,n]*val("x",(i,j,n)));rec("x_alloc",val("x",(i,j,n))-sum(val("alloc_boxes",(i,j,g,n)) for g in ship_groups(data,j)))
     for k in K:
      for g in ship_groups(data,j):
