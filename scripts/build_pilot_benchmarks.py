...
Groups encode size, POD, and height. `ActiveGroupsByShip` is the sparse ship-group index; `G` is its global union. A single-combination POD means at least one active size in that POD has exactly one height combination.

All three size classes target 25%-35% initial yard utilization. Scale growth is driven primarily by yard size, new-ship count, and active ship-group count rather than by progressively tighter yard occupancy.
