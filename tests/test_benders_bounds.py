...
def test_no_distance_double_counting():
 d=prepare_instance(get_data_tiny_benders());m,v,_=build_master_model(d,Weights());assert set(v)=={'x','alloc_boxes','block_use','eta'} and 'distance' not in str(m.getObjective()).lower()
...
def test_phase3_inherits_phase1_cuts():
 d=prepare_instance(get_data_tiny_benders());r=solve_true_benders_pipeline(d,Weights(),phase1_time=2,lns_time=.1,phase3_time=2,root_cut_time=1,mip_gap=0);assert r['cuts_inherited_by_phase3']>0
...
def test_root_prepass_improves_tiny_bound():
 d=prepare_instance(get_data_tiny_benders());r=root_lp_prepass(d,Weights(),BendersCutPool(),max_iters=20,time_limit=2);assert r['root_final_bound']>=r['root_initial_bound'] and r['root_cut_count']>0
