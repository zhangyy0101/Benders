...
def test_all_attribute_metrics_use_same_scope():
    d,c,final=_ref("final"); _d,_c,horizon=_ref("horizon"); assert final["start_attribute_score"]==attribute_score(d,Weights(),raw_components(d,c["solution"],"final"),"final"); assert horizon["start_attribute_score"]==attribute_score(d,Weights(),raw_components(d,c["solution"],"horizon"),"horizon")
