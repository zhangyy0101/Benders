...
def audit_instance(d,expected_digest=None,size_class=None):
 ...
 counts=[len(ship_groups(d,j)) for j in d["J_new"]];pods=[len(d["ActivePODsByShip"][j]) for j in d["J_new"]];ck("pod_counts",all(r["pods"][0]<=x<=r["pods"][1] for x in pods),pods);ck("positive_group_counts",all(r["groups"][0]<=x<=r["groups"][1] for x in counts),counts);ck("single_combination_pod",m["single_combination_pod_ratio"]>0)
...
def write_manifests(root,rows):
 fields=list(rows[0]);(root/"manifest.json").write_text(json.dumps({"pilot_version":PILOT_VERSION,"final_algorithm_frozen":False,"instances":rows},indent=2,ensure_ascii=False)+"\n",encoding="utf-8")
