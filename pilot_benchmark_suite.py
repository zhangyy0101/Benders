...
RANGES={"small":{"groups":(8,14),"pods":(5,5),"overlap":(.25,.50),"pressure":(.25,.45),"initial_utilization":(.25,.35)},"medium":{"groups":(14,24),"pods":(6,7),"overlap":(.50,.75),"pressure":(.45,.65),"initial_utilization":(.25,.35)},"large":{"groups":(22,36),"pods":(8,8),"overlap":(.75,1),"pressure":(.60,.80),"initial_utilization":(.25,.35)}}

def pilot_specs():
 result={};settings={
  "small":dict(num_blocks=3,bays_per_block=4,num_new_ships=2,num_old_ships=3,initial_utilization_range=(.25,.35),arrival_boxes_per_ship_range=(50,65),group_profile="pilot2_small",pod_count_range=(5,5),positive_group_count_range=(8,14),single_combination_pod_ratio_range=(.30,.50),arrival_overlap_ratio_range=(.25,.50),pressure_positive_ratio_range=(.25,.45)),
  "medium":dict(num_blocks=6,bays_per_block=6,num_new_ships=3,num_old_ships=4,initial_utilization_range=(.25,.35),arrival_boxes_per_ship_range=(95,115),group_profile="pilot2_medium",pod_count_range=(6,7),positive_group_count_range=(14,24),single_combination_pod_ratio_range=(.20,.40),arrival_overlap_ratio_range=(.50,.75),pressure_positive_ratio_range=(.45,.65)),
  "large":dict(num_blocks=10,bays_per_block=8,num_new_ships=5,num_old_ships=7,initial_utilization_range=(.25,.35),arrival_boxes_per_ship_range=(100,125),group_profile="pilot2_large",pod_count_range=(8,8),positive_group_count_range=(22,36),single_combination_pod_ratio_range=(.10,.30),arrival_overlap_ratio_range=(.75,1),pressure_positive_ratio_range=(.60,.80))}
...
def audit_instance(d,expected_digest=None,size_class=None):
 ...
 initial_utilization=sum(d["initial_inventory_data"].values())/sum(v["cap"] for v in d["I"].values());ck("digest",expected_digest is None or instance_digest(d)==expected_digest);ck("12_periods",len(d["N"])==12);ck("low_initial_utilization",r["initial_utilization"][0]<=initial_utilization<=r["initial_utilization"][1],initial_utilization);ck("old_outbound_feasible",sim["max_capacity_violation"]<=1e-6 and max(sim["unserved_outbound"].values(),default=0)<=1e-6)
