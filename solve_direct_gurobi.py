...
def solve_direct_gurobi(data,weights,*,time_limit=30,mip_gap=.03,threads=1,alloc_domain="integer",add_valid_inequalities=True,concentration_enabled=True,seed=0,verbose=False,start_solution=None):
    ...
    remaining=max(0,deadline-time.perf_counter());t=time.perf_counter();events=[];last={"ub":None,"lb":None};incumbents=[0]
...
def solve_direct_alns_pipeline(data,weights,*,total_time=180,warm_start_time_share=.15,alns_time_share=.25,mip_gap=.03,threads=1,alloc_domain="integer",add_valid_inequalities=True,concentration_enabled=True,seed=0,lns_options=None):
    ...
    trace=[];offset=0.0
    if warm["ok"]:trace.append({"time":warm["runtime"],"phase":"warm","source":"warm_incumbent","ub":warm["ub"],"lb":None})
    offset+=warm["runtime"]
    for item in alns.get("anytime_trace",[]):trace.append({**item,"time":offset+item["time"]})
    offset+=alns.get("runtime",0)
    for item in main.get("anytime_trace",[]):trace.append({**item,"time":offset+item["time"]})
    runtime=time.perf_counter()-pipeline_started
    return {"anytime_trace":canonicalize(trace,runtime,ub,lb),"ok":bool(candidates),"algorithm":"direct_gurobi_warm_alns","status_name":main["status_name"],"ub":ub,"lb":lb,"gap":None if ub is None or lb is None else max(0,(ub-lb)/max(abs(ub),1e-9)),"runtime":runtime,"nodes":main["nodes"],"solution":best.get("solution"),"components":best.get("components"),"time_budget":{"total":total,"warm":warm_budget,"alns":alns_budget,"main":main_budget,"main_actual_limit":main_limit},"warm_start":{k:warm.get(k) for k in ("ok","status_name","ub","lb","gap","runtime")},"alns":{k:v for k,v in alns.items() if k!="best_solution"},"main_solve":{k:main.get(k) for k in ("ok","status_name","ub","lb","gap","runtime","nodes")}}
def parser():
    p=argparse.ArgumentParser();source=p.add_mutually_exclusive_group();source.add_argument("--instance",choices=list_builtin_instances(),default="3new6old");source.add_argument("--instance-file");p.add_argument("--time",type=float,default=30);p.add_argument("--mip-gap",type=float,default=.03);p.add_argument("--threads",type=int,default=1);p.add_argument("--alloc-domain",choices=("integer","continuous"),default="integer");p.add_argument("--concentration",action=argparse.BooleanOptionalAction,default=True);p.add_argument("--concentration-weight",type=float,default=10);p.add_argument("--concentration-mode",choices=("joint-group-bay",),default="joint-group-bay");p.add_argument("--handling-rate-scale",type=float,default=1);p.add_argument("--old-outbound-release-policy",choices=("proportional","legacy_sorted","conservative"),default="conservative");p.add_argument("--no-valid-inequalities",action="store_true");p.add_argument("--output-root",default="outputs");p.add_argument("--quiet",action="store_true");return p
def main():
