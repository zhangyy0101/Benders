...
def solve_direct_gurobi(data,weights,*,time_limit=30,mip_gap=.03,threads=1,alloc_domain="integer",add_valid_inequalities=True,seed=0,verbose=False,start_solution=None):
    m,v,_=build_monolithic_model(data,weights,alloc_domain=alloc_domain,add_valid_inequalities=add_valid_inequalities);m.Params.OutputFlag=int(verbose);m.Params.TimeLimit=float(time_limit);m.Params.MIPGap=float(mip_gap);m.Params.Threads=int(threads or 1);m.Params.Seed=int(seed)
    if start_solution:
        for name,values in start_solution.items():
            for k,value in values.items():
                if k in v.get(name,{}):v[name][k].Start=value
    t=time.perf_counter();m.optimize();runtime=time.perf_counter()-t;solution=extract_solution(v) if m.SolCount else None;evaluation=evaluate_solution(data,weights,solution) if solution else None
...
def main():
    p=argparse.ArgumentParser();p.add_argument("--instance",choices=INSTANCES,default="3new6old");p.add_argument("--time",type=float,default=30);p.add_argument("--mip-gap",type=float,default=.03);p.add_argument("--threads",type=int,default=1);p.add_argument("--alloc-domain",choices=("integer","continuous"),default="integer");p.add_argument("--handling-rate-scale",type=float,default=1);p.add_argument("--no-valid-inequalities",action="store_true");p.add_argument("--output-root",default="outputs");p.add_argument("--quiet",action="store_true");a=p.parse_args();d=prepare_instance(INSTANCES[a.instance](),a.handling_rate_scale);r=solve_direct_gurobi(d,Weights(),time_limit=a.time,mip_gap=a.mip_gap,threads=a.threads,alloc_domain=a.alloc_domain,add_valid_inequalities=not a.no_valid_inequalities,verbose=not a.quiet);out=os.path.join(a.output_root,f"direct_{datetime.now():%Y%m%d_%H%M%S}_{a.instance}");os.makedirs(out,exist_ok=True);json.dump({k:v for k,v in r.items() if k!="solution"},open(os.path.join(out,"summary.json"),"w",encoding="utf8"),indent=2,default=str);print({k:r[k] for k in ("status_name","ub","lb","gap","runtime")});return 0 if r["ok"] else 2
