...
def parser():
    p=argparse.ArgumentParser(description="Generic/legacy multi-method runner; candidate runs must use scripts/run_candidate_experiments.py");p.add_argument("--suite-dir",default="benchmarks/paper_exp_v1_pilot");p.add_argument("--instances",nargs="+");p.add_argument("--methods",nargs="+",choices=METHODS,default=["direct","bbc_candidate"]);p.add_argument("--algorithm-configs",nargs="+",choices=list_algorithm_configurations(),default=["bbc_full_current"]);p.add_argument("--seeds",nargs="+",type=int,default=[0]);p.add_argument("--budget",type=float,default=300);p.add_argument("--threads",type=int,default=1);p.add_argument("--mip-gap",type=float,default=.03);p.add_argument("--alloc-domain",choices=("integer","continuous"),default="integer");p.add_argument("--handling-rate-scale",type=float,default=1.0);p.add_argument("--old-outbound-release-policy",choices=("proportional","legacy_sorted","conservative"),default="proportional");p.add_argument("--output",default="experiments/pilot");p.add_argument("--resume",action="store_true");p.add_argument("--rerun-failed",action="store_true");p.add_argument("--save-solutions",action="store_true");return p
