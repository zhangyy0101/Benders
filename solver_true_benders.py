...
def solve_bbc_phase(data,weights,*,time_limit,mip_gap=.03,alloc_domain="integer",add_valid_inequalities=True,aggregate_recourse_lb=True,analytic_recourse_lb=True,concentration_enabled=True,cut_strategy="standard",node_cuts=False,node_cut_limit=100,node_separation_policy="root-only",node_separation_interval=20,callback_time_share_limit=.4,seed=0,threads=1,numeric_focus=1,cut_pool=None,start_solution=None,start_solution_source=None,origin_prefix="phase1",warm_start=True):
    ...
    if start_solution:
        feasibility=validate_solution(data,start_solution,alloc_domain=alloc_domain,add_valid_inequalities=add_valid_inequalities,concentration_enabled=concentration_enabled)
        if not feasibility["feasible"]:raise RuntimeError(f"supplied start solution failed checker: {feasibility}")
    ...
    if warm:
        ...
        if status==GRB.OPTIMAL:
            record=oracle.build_optimality_cut(point,"initial");recourse_cache[master_point_cache_key(point,alloc_domain)]={"status":status,"q":oracle.objective_value(),"record":record,"recourse":oracle.solution()};stats["cache_seeded_entries"]=1;stats["initial_cut_tightness"]=record.value_at(point)
            ...
            initial_phase="repair" if start_solution_source=="agfr" else "warm";initial_source="agfr_incumbent" if start_solution_source=="agfr" else "warm_incumbent";anytime.append({"time":time.perf_counter()-phase_started,"phase":initial_phase,"source":initial_source,"ub":warm["ub"],"lb":None})
            ...
            stats["initial_cutoff"]=warm["ub"]+1e-6;stats["start_solution_source"]=start_solution_source or "warm"
            core_point=point
...
def solve_true_benders_pipeline(data,weights,*,total_core_time=60,root_time_share=.05,warm_start_time_share=.15,alns_time_share=.25,main_bbc_time_share=.55,mip_gap=.03,alloc_domain="integer",add_valid_inequalities=True,aggregate_recourse_lb=True,analytic_recourse_lb=True,concentration_enabled=True,cut_strategy="standard",root_prepass=True,root_cut_max_iters=100,root_cut_time=None,root_cut_relative_improvement_tol=1e-4,root_cut_violation_tol=1e-6,root_cut_stall_iters=5,node_cuts=False,node_cut_limit=100,node_separation_policy="root-only",node_separation_interval=20,callback_time_share_limit=.4,enable_alns=True,enable_phase3=True,seed=0,threads=1,warm_start=True,numeric_focus=1,lns_options=None,primal_repair=False,primal_repair_time_share=.08,primal_repair_min_seconds=2.0,primal_repair_max_seconds=20.0,primal_repair_guide_share=.25,primal_repair_mip_gap=.05,primal_repair_max_expansions=2):
    ...
    active_concentration=bool(concentration_enabled and weights.master.concentration>0);repair_planned=total*float(primal_repair_time_share);repair_limit=min(max(float(primal_repair_min_seconds),repair_planned),float(primal_repair_max_seconds),remaining_time(deadline)) if primal_repair else 0.0
    if primal_repair and repair_limit>0:
        from solver_fix_and_repair import solve_aggregate_guided_fix_and_repair
        repair=solve_aggregate_guided_fix_and_repair(data,weights,time_limit=repair_limit,threads=threads,seed=seed,mip_gap=primal_repair_mip_gap,alloc_domain=alloc_domain,concentration_enabled=active_concentration,guide_time_share=primal_repair_guide_share,max_expansions=primal_repair_max_expansions,add_valid_inequalities=False)
    else:repair={"ok":False,"status_name":"disabled" if not primal_repair else "deadline_exhausted","runtime":0.0,"ub":None,"solution":None,"guide":{},"candidate_domain":None,"repair_attempts":[],"selected_attempt":None,"oracle_consistent":None,"anytime_trace":[]}
    domain_diag=(repair.get("candidate_domain") or {}).get("diagnostics",{});selected_repair=None if repair.get("selected_attempt") is None else repair["repair_attempts"][repair["selected_attempt"]]
    repair_phase={"enabled":bool(primal_repair),"method":"aggregate_guided_fix_and_repair" if primal_repair else "none","ok":bool(repair.get("ok")),"status":repair.get("status_name"),"runtime":repair.get("runtime",0.0),"ub":repair.get("ub"),"guide_source":repair.get("guide",{}).get("source"),"guide_runtime":repair.get("guide",{}).get("runtime"),"selected_expansion_level":None if selected_repair is None else selected_repair.get("expansion_level"),"candidate_pair_ratio":domain_diag.get("candidate_pair_ratio"),"restricted_variable_ratio":None if selected_repair is None else selected_repair.get("restricted_variable_count",0)/max(1,selected_repair.get("estimated_full_variable_count",1)),"repair_attempt_count":len(repair.get("repair_attempts",[])),"oracle_consistent":repair.get("oracle_consistent"),"fallback_reason":None if repair.get("ok") else repair.get("status_name")}
    ...
    inherited_before=len(pool.records);main_time=remaining_time(deadline);main_start=repair.get("solution") if repair.get("ok") else alns["best_solution"];main_source="agfr" if repair.get("ok") else None;p3=solve_bbc_phase(data,weights,time_limit=main_time,mip_gap=mip_gap,alloc_domain=alloc_domain,add_valid_inequalities=add_valid_inequalities,aggregate_recourse_lb=aggregate_recourse_lb,analytic_recourse_lb=analytic_recourse_lb,concentration_enabled=active_concentration,cut_strategy=cut_strategy,node_cuts=node_cuts,node_cut_limit=node_cut_limit,node_separation_policy=node_separation_policy,node_separation_interval=node_separation_interval,callback_time_share_limit=callback_time_share_limit,seed=seed+1,threads=threads,numeric_focus=numeric_focus,cut_pool=pool,start_solution=main_start,start_solution_source=main_source,origin_prefix="main",warm_start=False) if enable_phase3 and main_time>0 else {"ok":False,"disabled":True,"reason":"global_deadline_exhausted" if main_time<=0 else "disabled","lb":None,"runtime":0,"nodes":0}
    pipeline_trace=[];offset=0.0
    for item in repair.get("anytime_trace",[]):
        if item.get("phase")=="guide":pipeline_trace.append({**item,"time":float(item["time"])})
        elif item.get("ub") is not None:pipeline_trace.append({**item,"phase":"repair","source":"agfr_incumbent","time":float(item["time"])})
    offset+=float(repair.get("runtime",0));root_trace=phase0.get("iteration_trace",[]);root_runtime=float(phase0.get("root_cut_runtime",0))
    for idx,item in enumerate(root_trace):pipeline_trace.append({"time":offset+root_runtime*(idx+1)/max(1,len(root_trace)),"phase":"root","source":"completed_root_lp","ub":None,"lb":item.get("master_lp_obj")})
    offset+=root_runtime
    ...
    for item in p3.get("anytime_trace",[]):
        if item.get("source")!="agfr_incumbent":pipeline_trace.append({**item,"time":offset+float(item["time"])})
    candidates=[]
    if repair.get("ok"):candidates.append(("agfr",repair["ub"],repair["solution"]))
    ...
    if not candidates:
        runtime=time.perf_counter()-pipeline_started;return {"anytime_trace":canonicalize(pipeline_trace,runtime,None,p3.get("lb")),"ok":False,"algorithm":"true_bbc_alns","workflow":"single_main_bbc","phase_primal_repair":repair_phase,"phase0_root_prepass":phase0,"phase1_initialization":p1,"phase1_bbc":p1,"phase2_alns":alns,"phase3_bbc":p3,"runtime":runtime}
    ...
    return {"anytime_trace":canonicalize(pipeline_trace,runtime,ub,lb),"ok":True,"algorithm":"true_bbc_alns_joint_bay_concentration","workflow":"single_main_bbc","runtime":runtime,"objective":{"open_weight":weights.master.x,"concentration_weight":weights.master.concentration,"distance_weight":weights.sub.dist,"balance_weight":weights.sub.balance,"conflict_weight":weights.sub.conflict},"concentration":{"mode":"joint_group_bay","available":con["available"],"enabled":con["enabled"],"status":con["status"],"raw_used_bays":con["raw_used_bays"],"normalized":con["normalized"],"cost":evaluation["concentration_cost"],"scale":con["scale"],"positive_ship_groups":len(con["positive_ship_groups"]),"used_bays_total":con["used_bays_total"]},"time_budget":{"total_core_time":total,"planned_repair":repair_planned if primal_repair else 0.0,"repair_limit":repair_limit,"planned_root":root_time,"planned_warm":warm_time,"planned_alns":alns_budget,"main_bbc_actual_limit":main_time},"phase_primal_repair":repair_phase,"phase0_root_prepass":phase0,"phase1_initialization":p1,"phase1_bbc":p1,"phase2_alns":alns,"phase3_bbc":p3,"core_best":{"ub":ub,"lb":lb,"gap":None if lb is None else max(0,(ub-lb)/max(abs(ub),1e-9)),"components":evaluation,"solution":solution,"solution_source":source,"ub_source":"exact_first_stage_plus_global_recourse","lb_source":"benders_master_bound","feasibility_report":validate_solution(data,solution,alloc_domain=alloc_domain,add_valid_inequalities=add_valid_inequalities,concentration_enabled=active_concentration)},"cuts_inherited_by_phase3":inherited_before,"new_phase3_cuts":len(pool.records)-inherited_before,"total_unique_cuts":len(pool.records)}


def solve_bbc_then_repair_pipeline(data,weights,*,total_core_time=60,primal_repair_time_share=.10,
                                   primal_repair_min_seconds=3.0,primal_repair_max_seconds=12.0,
                                   primal_repair_mip_gap=.03,primal_repair_max_expansions=2,
                                   threads=1,seed=0,alloc_domain="integer",add_valid_inequalities=False,
                                   concentration_enabled=True,**bbc_options):
    """Run exact BBC first, then improve its incumbent in a support-preserving restricted model."""
    started=time.perf_counter();total=float(total_core_time);reserve=min(max(float(primal_repair_min_seconds),total*float(primal_repair_time_share)),float(primal_repair_max_seconds),total)
    bbc=solve_true_benders_pipeline(data,weights,total_core_time=max(0,total-reserve),threads=threads,seed=seed,alloc_domain=alloc_domain,add_valid_inequalities=add_valid_inequalities,concentration_enabled=concentration_enabled,primal_repair=False,**bbc_options)
    incumbent=bbc.get("core_best",{}).get("solution");remaining=max(0,total-(time.perf_counter()-started));repair=None
    if incumbent is not None and remaining>0:
        from solver_fix_and_repair import solve_aggregate_guided_fix_and_repair
        repair=solve_aggregate_guided_fix_and_repair(data,weights,time_limit=remaining,threads=threads,seed=seed,mip_gap=primal_repair_mip_gap,alloc_domain=alloc_domain,concentration_enabled=concentration_enabled,max_expansions=0,add_valid_inequalities=False,incumbent_solution=incumbent,canonical_oracle=False)
    if repair is None:repair={"ok":False,"status_name":"no_bbc_incumbent" if incumbent is None else "deadline_exhausted","runtime":0.0,"ub":None,"solution":None,"guide":{},"candidate_domain":None,"repair_attempts":[],"selected_attempt":None,"oracle_consistent":None,"anytime_trace":[]}
    domain=(repair.get("candidate_domain") or {}).get("diagnostics",{});selected=None if repair.get("selected_attempt") is None else repair["repair_attempts"][repair["selected_attempt"]]
    phase={"enabled":True,"method":"bbc_incumbent_guided_fix_and_repair","ok":bool(repair.get("ok")),"status":repair.get("status_name"),"runtime":repair.get("runtime",0.0),"ub":repair.get("ub"),"ub_source":None if selected is None else selected.get("ub_source","exact_global_recourse"),"guide_source":repair.get("guide",{}).get("source"),"guide_runtime":repair.get("guide",{}).get("runtime"),"selected_expansion_level":None if selected is None else selected.get("expansion_level"),"candidate_pair_ratio":domain.get("candidate_pair_ratio"),"restricted_variable_ratio":None if selected is None else selected.get("restricted_variable_count",0)/max(1,selected.get("estimated_full_variable_count",1)),"repair_attempt_count":len(repair.get("repair_attempts",[])),"oracle_consistent":repair.get("oracle_consistent"),"fallback_reason":None if repair.get("ok") else repair.get("status_name")}
    trace=list(bbc.get("anytime_trace",[]));offset=float(bbc.get("runtime",0.0))
    for item in repair.get("anytime_trace",[]):
        if item.get("ub") is not None:trace.append({**item,"time":offset+float(item["time"]),"phase":"repair","source":"agfr_incumbent"})
    best=bbc.get("core_best",{});old_ub=best.get("ub")
    if repair.get("ok") and (old_ub is None or repair["ub"]<old_ub-1e-7):
        solution=repair["solution"];evaluation=evaluate_solution(data,weights,solution,alloc_domain=alloc_domain,concentration_enabled=concentration_enabled);lb=best.get("lb");bbc["core_best"]={**best,"ub":repair["ub"],"gap":None if lb is None else max(0,(repair["ub"]-lb)/max(abs(repair["ub"]),1e-9)),"components":evaluation,"solution":solution,"solution_source":"agfr","feasibility_report":validate_solution(data,solution,alloc_domain=alloc_domain,add_valid_inequalities=add_valid_inequalities,concentration_enabled=concentration_enabled)}
    runtime=time.perf_counter()-started;final=bbc.get("core_best",{});bbc.update({"runtime":runtime,"workflow":"bbc_then_incumbent_guided_repair","phase_primal_repair":phase,"anytime_trace":canonicalize(trace,runtime,final.get("ub"),final.get("lb"))});bbc.setdefault("time_budget",{}).update({"total_core_time":total,"planned_repair":reserve,"repair_limit":remaining,"bbc_limit":max(0,total-reserve)});return bbc
