...
def run_method(method,data,weights,configuration,*,budget,threads,mip_gap,alloc_domain,seed):
    if method=="direct":result=solve_direct_gurobi(data,weights,time_limit=budget,threads=threads,mip_gap=mip_gap,alloc_domain=alloc_domain,seed=seed)
        ...
        c=configuration;s=c["phase_shares"];result=solve_true_benders_pipeline(data,weights,total_core_time=budget,threads=threads,mip_gap=mip_gap,alloc_domain=alloc_domain,seed=seed,root_time_share=s["root"],warm_start_time_share=s["warm"],alns_time_share=s["alns"],main_bbc_time_share=s["main"],root_prepass=c["root_prepass"],warm_start=c["warm_start"],enable_alns=c["alns"],aggregate_recourse_lb=c["aggregate_recourse_lb"],analytic_recourse_lb=c["analytic_recourse_lb"],add_valid_inequalities=c["valid_inequalities"],node_cuts=c["node_cuts"],cut_strategy=c["cut_strategy"],lns_options=c["alns_parameters"],primal_repair=c.get("primal_repair",False),primal_repair_time_share=c.get("primal_repair_time_share",.08),primal_repair_min_seconds=c.get("primal_repair_min_seconds",2),primal_repair_max_seconds=c.get("primal_repair_max_seconds",20),primal_repair_guide_share=c.get("primal_repair_guide_share",.25),primal_repair_mip_gap=c.get("primal_repair_mip_gap",.05),primal_repair_max_expansions=c.get("primal_repair_max_expansions",2))
