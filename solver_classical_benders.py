...
def solve_classical_benders(data,weights,*,time_limit,mip_gap=0.0,alloc_domain="integer",concentration_enabled=True,seed=0,threads=1,tolerance=1e-6,max_iterations=None):
    started=time.perf_counter();deadline=started+max(0.0,float(time_limit));pool=BendersCutPool();model,variables,_=build_master_model(data,weights,alloc_domain=alloc_domain,add_valid_inequalities=False,aggregate_recourse_lb=False,analytic_recourse_lb=False,concentration_enabled=concentration_enabled);model.Params.OutputFlag=0;model.Params.Threads=int(threads or 1);model.Params.Seed=int(seed);model.Params.MIPGap=float(mip_gap);oracle=GlobalRecourseOracle(data,weights);best_ub=float("inf");best_solution=None;best_evaluation=None;global_lb=None;trace=[];optimality_cuts=feasibility_cuts=cache_hits=0;cache={};first_feasible_time=best_solution_time=None;termination="unknown";iteration=0
    while True:
