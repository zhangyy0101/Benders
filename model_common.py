...
def add_common_master_valid_inequalities(model,data,variables,*,enabled=True):
    ...
    x,alloc=variables["x"],variables["alloc_boxes"];I,J,G,N=data["I_list"],data["J_new"],groups(data),data["N"];modes={i:int(data["Fixed_Bay_Mode"][i]) for i in I};alpha=float(data["Alpha"]);added=[]
    for j in J:
      for size in data["S"]:
