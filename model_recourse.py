...
class GlobalRecourseOracle:
    def __init__(self,data,weights):
        ...
        m=self.model;self.din=m.addVars([(j,g,i,n) for j,g in pairs for i in I for n in N],lb=0,name="din");self.inv=m.addVars([(j,g,i,n) for j,g in pairs for i in I for n in N],lb=0,name="inv");self.share=m.addVars([(j,k,g,n) for j,g in pairs for k in K for n in N],lb=0,name="in_share");self.total=m.addVars(K,N,lb=0,name="in_total");self.avg=m.addVars(N,lb=0,name="avg");self.bal=m.addVars(K,N,lb=0,name="g_bal");self.rhs_records=[];self.storage={};self.handling={};alpha=float(data["Alpha"])
        ...
        for j in J:
         for i in I:
          for n in N:
           coeff=float(data["Bay_Handling_Rate"][i,n])*float(data["Intervals"][n]["dur"]);key=(i,j,n);self.handling[key]=add(alpha*gp.quicksum(self.din[j,g,i,n] for g in ship_groups(data,j)),"<=",0,f"handling_{i}_{j}_{n}",{"constant":0,"x":{key:coeff},"alloc":{}})
         for k in K:
